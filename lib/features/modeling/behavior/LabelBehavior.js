...
function LabelSupport(eventBus, modeling, bpmnFactory) {
  ...
  this.postExecute([ 'connection.updateWaypoints' ], function(e) {
    ...
    var oldWayLen = LabelUtil.getWaypointsLength(oldWaypoints);
    var newWayLen = LabelUtil.getWaypointsLength(newWaypoints);

    console.log(oldWayLen, newWayLen);

    for(var i=0; i < oldWaypoints - 1; i++) {

    }


    // var waypointsMid = LabelUtil.getWaypointsMid(connection.waypoints);


    return;
    //
    // console.log(waypointsMid);
    //
    // var newPos = LabelUtil.getNewFlowLabelPosition(label, connection.waypoints);
    //
    // var deltaPos = {
    //   x: newPos.x - label.x,
    //   y: newPos.y - label.y,
    // }
    //
    // modeling.moveShape(label, deltaPos);
