...
function getWaypointsLength(waypoints) {
  ...
  for (var i=0; i < waypoints.length - 1; i++) {
    ...
    // calculate distance d between points p1 and p2
    var d = GeometricUtil.getDistancePointPoint(p1, p2);
    length += d;
  }
...
module.exports.getWaypointsLength = getWaypointsLength;
...
function getNewFlowLabelPosition(label, waypoints) {
  ...
  for(var i=0; i < waypoints.length-1; i++) {
    ...
    if (!min) {
      min = GeometricUtil.getDistancePointLine(label, [ first, second ]);
      minLine.first = first;
      minLine.second = second;
      continue;
    }

    var newmin = GeometricUtil.getDistancePointLine(label, [ first, second ]);
